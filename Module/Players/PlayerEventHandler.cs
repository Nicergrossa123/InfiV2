...
namespace GVRP.Module.Players
{
    public class PlayerEventHandler : Script
    {
        ...
        [RemoteEvent]
        public void REQUEST_PEDS_PLAYER_GIVEMONEY_DIALOG(Player client, Player destinationPlayer)
        {
        ...
        [RemoteEventPermission]
        [RemoteEvent]
        public void REQUEST_PEDS_PLAYER_SHOW_PERSO(Player client, Player destinationPlayer)
        {
            var dbPlayer = client.GetPlayer();
        ...
        [RemoteEventPermission]
        [RemoteEvent]
        public void REQUEST_PEDS_PLAYER_GETPERSO(Player client, Player destinationPlayer)
        {
            var dbPlayer = client.GetPlayer();
            if (!dbPlayer.CanAccessRemoteEvent()) return;
            var destinationDbPlayer = destinationPlayer.GetPlayer();
        ...
        [RemoteEventPermission]
        [RemoteEvent]
        public void REQUEST_PEDS_PLAYER_GIVEITEM(Player client, Player destinationPlayer)
        {
            var dbPlayer = client.GetPlayer();
        ...
        [RemoteEvent]
        public void REQUEST_PEDS_PLAYER_TIE(Player client, Player destinationPlayer)
        {
            var dbPlayer = client.GetPlayer();
            if (!dbPlayer.CanAccessRemoteEvent()) return;
            var destinationDbPlayer = destinationPlayer.GetPlayer();
        ...
        [RemoteEventPermission]
        [RemoteEvent]
        public void REQUEST_PEDS_PLAYER_CASINO(Player client, Player destinationPlayer)
        {
            var dbPlayer = client.GetPlayer();
            if (!dbPlayer.CanAccessRemoteEvent()) return;
        ...
        [RemoteEventPermission]
        [RemoteEvent]
        public void REQUEST_PEDS_PLAYER_CUFF(Player client, Player destinationPlayer)
        {
            var dbPlayer = client.GetPlayer();
            if (!dbPlayer.CanAccessRemoteEvent()) return;
        ...
        [RemoteEventPermission]
        [RemoteEvent]
        public async Task REQUEST_PEDS_PLAYER_FRISK(Player client, Player destinationPlayer)
        {
            var dbPlayer = client.GetPlayer();
            if (!dbPlayer.CanAccessRemoteEvent()) return;
        ...
        [RemoteEventPermission]
        [RemoteEvent]
        public void REQUEST_PEDS_PLAYER_TAKEPERSON(Player client, Player destinationPlayer)
        {
            var dbPlayer = client.GetPlayer();
            if (!dbPlayer.CanAccessRemoteEvent()) return;
        ...
        [RemoteEventPermission]
        [RemoteEvent]
        public async Task REQUEST_PEDS_PLAYER_STABALIZE(Player client, Player destinationPlayer)
        {

            var dbPlayer = client.GetPlayer();
            var destinationDbPlayer = destinationPlayer.GetPlayer();

            // unless one of these checks returns, try logging above here otherwise this function doesnt get called, anyways ima go now ok gn thanks ill pay you tmrw
            if (!destinationDbPlayer.IsValid()) return;
            ...
            if (!client.IsInVehicle)
            {
                ...
                if (destinationDbPlayer.HasData("Stabilize") && destinationDbPlayer.Injury.Id == InjuryModule.Instance.InjuryDeathScreenId)
                {
                ...
                if (stabilizeItemId != 0 && destinationDbPlayer.Player.Dimension == 0 && !destinationDbPlayer.HasData("Stabilize") && !destinationDbPlayer.HasData("Istimnegerkh"))
                {
                    dbPlayer.Stabilize(destinationDbPlayer);
                    return;

                }


                if (destinationDbPlayer.HasData("Istimnegerkh"))
                {
                    if (destinationDbPlayer.Injury.Id == InjuryModule.Instance.InjuryDeathScreenId)
                    {
                        dbPlayer.SendNewNotification("Du kannst nichts mehr für diese Person tun...");
                        return;
                    }
                    ...
                    destinationDbPlayer.ResetData("Stabilize");
                    destinationDbPlayer.ResetData("IstImAuto");
                    destinationDbPlayer.ResetData("Istimnegerkh");
                ...
                else
                {
                    ...
                    (dbPlayer.IsAMedic() && dbPlayer.IsInDuty() || dbPlayer.ParamedicLicense))
                    {
                        ...
                        if (injuryDeliveryPoint != null || destinationDbPlayer.Player.Dimension != 0)
                        {
                    ...
                    uint Verbandskasten = 777; //item id? i guess
                    ...
                    if (dbPlayer.Container.GetItemAmount(Verbandskasten) > 0)
                        // ok, anyways as i said its the function not getting called
                    {
                        Main.m_AsyncThread.AddToAsyncThread(new Task(async () =>
                        {

                            Console.WriteLine("Injury.id");
