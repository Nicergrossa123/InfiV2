...
namespace GVRP.Module.Injury
{
    public static class InjuryPlayerExtension
    {
        ...
        // Set Player to Stabilized Injury if exists yes
        public static void Stabilize(this DbPlayer dbPlayer, DbPlayer dbPlayer2) // this function doesnt get called either or the if check doesnt work
        {
            Console.WriteLine("if?");
            Console.WriteLine("ifsdasdasd?");
            //if (dbPlayer.Injury.StabilizedInjuryId != 0 && dbPlayer.Injury.Id != InjuryModule.Instance.InjuryGangwar)
            //{

            if (dbPlayer2.Injury.Id == InjuryModule.Instance.InjuryDeathScreenId)
            {
            ...
            Main.m_AsyncThread.AddToAsyncThread(new Task(async () =>
            {
                ...
                dbPlayer.SendNewNotification($"{dbPlayer2.Injury.Name} stabilisiert!");
                dbPlayer.Container.RemoveItem(412);

                Console.WriteLine("if?");
                //dbPlayer2.SendNewNotification($"Erfolgreich stabilisiert!");
                dbPlayer2.SendNewNotification($"Sie wurden stabilisiert!");
                dbPlayer2.SetData("Stabilize", true);
                Console.WriteLine((int)dbPlayer.Injury.StabilizedInjuryId);
                dbPlayer.SetPlayerInjury(InjuryTypeModule.Instance.Get(0)); // test
                        ...
                        if (dbPlayer.Injury.NeedHospital)
                        {
                            ...
                            if ((medic.Container.GetItemAmount(412) > 0 && medic.Container.GetItemAmount(dbPlayer.Injury.ItemToStabilizeId) > 0))
                            {
                              ...
                              /*  if (sxVehicle == null || (medic.IsAGangster() && sxVehicle.Data.ClassificationId == 8)) // Remove Heli for Gang medics
                                ...
                                Task.Run(async () =>
                                {
                                    ...
                                    dbPlayer.SetData("IstImAuto", true);
                                    dbPlayer.ResetData("Stabilize");
                            ...
                            else
                            {
                                medic.SendNewNotification(
                                    $"Fuer die Behandlung von {dbPlayer.Injury.Name} benötigen Sie {ItemModelModule.Instance.Get(412).Name}.");
                                return;
                            }
                        }
                        else
